...
int init_module(void)
{
	...
	printk(KERN_INFO "In kfork_thread init_module \n");
	/* If we cant setup dbfs do not continue */
	if ( u_dbfs_init("kfork_thread") <  0 ){
		printk(KERN_INFO "In kfork_thread u_dbfs_init failed \n");
		return -1;
	}
...
{
	printk(KERN_INFO "cleanup kfork_thread num_probes=%ld\n",num_probes);
	u_dbfs_cleanup();	
}
MODULE_LICENSE("GPL");
