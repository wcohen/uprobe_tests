...
int init_module(void)
{
	...
	printk(KERN_INFO "In kexec init_module \n");
	...
	if ( u_dbfs_init("kexec") <  0 ){
		printk(KERN_INFO "In kexec u_dbfs_init failed \n");
		return -1;
	}
	...
	/* register all probes starting matching glob probe_[+]  */
		...
		usp=(struct uprobe *)kzalloc(sizeof(struct uprobe_consumer), GFP_USER);
...
{
	printk(KERN_INFO "cleanup kexec num_probes=%ld\n",num_probes);
	u_dbfs_cleanup();
}
MODULE_LICENSE("GPL");
